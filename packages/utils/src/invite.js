...
export function createInviteLink(origin, token) {
...
export function parseInviteTokenFromUrl(urlOrPath, fallbackOrigin = "http://localhost") {
    if (!isNonEmptyString(urlOrPath))
        return null;
    let parsed;
    try {
